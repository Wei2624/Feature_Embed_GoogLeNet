...
class CNN_Triplet_Metric(object):
    def __init__(self,sess):
        ...
        img_p = tf.placeholder(tf.float32, [None, 227, 227, 3])
        img_n = tf.placeholder(tf.float32, [None, 227, 227, 3])
        self.sess = sess
        ...
        w1test = self.sess.run(tf.get_default_graph().get_tensor_by_name("InceptionV1/Mixed_4b/Branch_0/Conv2d_0a_1x1/bias:0"))
        w0test = self.sess.run(tf.get_default_graph().get_tensor_by_name("InceptionV1/Mixed_4b/Branch_0/Conv2d_0a_1x1/weights:0"))
        test1 = np.sum(w0test[2:,:,:,1]) + w1test[1]
        print w0test.shape
        # print w_test
